...
- Default listen port: **30009**. `HOST` defaults to `0.0.0.0`. Redirect URL must match `TWITCH_REDIRECT_URI`. Non-localhost Twitch redirects need **https** in front of this process (reverse proxy or tunnel). Compose has `build: .` for Git/Portainer stacks (same as other apps). Env vars are interpolated; there is no `env_file: .env`.
