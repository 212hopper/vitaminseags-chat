...
3. `docker compose up -d --build` (or a Portainer **Git** stack — see [Deploy](#deploy)).
...
Use a **Git repository** stack, the same way as an app like dazn-clock (`build: .` with a Dockerfile in the repo):
